        ...
        <Reveal delay={0.15}>
          <div className="flex h-full flex-col justify-center border border-ink/15 bg-ink p-8 text-parchment">
            ...
            <p className="mt-4 font-display text-2xl font-medium leading-snug">
              The <span className="text-gold-soft">textbook</span> favourite-longshot bias
              — favourites outperformed their price.
            </p>
            <p className="mt-4 text-sm leading-relaxed text-parchment/70">
              Strong favourites earned their quote and then some: the 50–60% band
              converted at 80%, the 60–70% band at 87%, and the 70–80% band at a perfect
              100%. That is the textbook favourite-longshot bias — the market under-rated
              favourites rather than over-rating them. The lone crack sits in the middle
              of the ground: the 40–50% dead zone won just 33%, where France's upset of
              Spain and a clutch of level-at-90 knockout ties collected. France then
              turned Brazil over at 4.20; Zidane's last World Cup ended in a drawn final
              Italy won on penalties. Netted across 64 matches, the aggregate favourite
              bias of{" "}
              <span className="tnum text-gold-soft">
                {biasPct >= 0 ? "+" : ""}{biasPct.toFixed(1)}%
              </span>{" "}
              is positive — favourites won far more often than their price implied.
            </p>
          </div>
        </Reveal>
      </div>
