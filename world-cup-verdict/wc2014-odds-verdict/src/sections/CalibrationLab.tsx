        ...
        <Reveal delay={0.15}>
          <div className="flex h-full flex-col justify-center border border-ink/15 bg-ink p-8 text-parchment">
            ...
            <p className="mt-4 font-display text-2xl font-medium leading-snug">
              A <span className="text-gold-soft">bimodal</span> market — sharp at the top,
              a graveyard at the bottom.
            </p>
            <p className="mt-4 text-sm leading-relaxed text-parchment/70">
              Strong favourites — priced above 50% — won markedly more often than their
              quote, the 70–80% band converting at a perfect 100% and the 50–60% band at
              75%. That is the textbook favourite-longshot bias at the confident end. But
              the tightest calls collapsed entirely: in the 33–40% band, where the
              favourite was priced barely ahead, all twelve matches went against it — zero
              for twelve. Spain's opening demolition by the Netherlands and Brazil's
              Mineirazo against Germany sat in this violent middle. Netted across 64
              matches, the aggregate favourite bias of{" "}
              <span className="tnum text-gold-soft">
                {biasPct >= 0 ? "+" : ""}{biasPct.toFixed(1)}%
              </span>{" "}
              is small but hides a calibration curve that fell off a cliff below 50%.
            </p>
          </div>
        </Reveal>
      </div>
